# ledger — delta for add-ledger-and-run-logs
...
still exists in the collection; if it does not (the file was undone,
deleted, or moved), the entry is stale — the incoming file is processed
normally and a warning suggests `borax ledger rebuild`. Disk is the
source of truth; the ledger alone never vetoes an admission.

#### Scenario: Duplicate of an undone admission
- **WHEN** an incoming file matches a ledger entry whose recorded path
  no longer exists (its rename was undone)
