...
- [ ] 3.4 Confirm `cargo test` is green across the workspace

## 4. Re-ground the specifications

- [ ] 4.1 Verify the four delta files under
...
- [ ] 4.2 Run `openspec validate remove-undo --strict` and
      `python3 scripts/check-spec-deltas.py`; both must pass

## 5. Documentation

- [ ] 5.1 `README.md`: drop "and can undo" from the opening claim, and
...
- [ ] 5.2 `openspec/STATE.md`: update "What is built" so the
...
- [ ] 5.3 `CHANGELOG.md`: a `0.3.0` section recording the removal as
...
- [ ] 6.1 Bump the workspace version to `0.3.0`
- [ ] 6.2 Full suite green on Linux, macOS and Windows in CI
- [ ] 6.3 Archive the change with `openspec archive remove-undo` and
